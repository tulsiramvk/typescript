...
const AddActivity = (props) => {
    ...
    const [formData, setFormData] = useState({
        type: data?.type || 'Call', subject: data?.subject || '', date: data?.date || new Date(), related_to: data?.relatedToType || "", related_id: data?.relatedToId || '', status: '', notes: data?.notes || '', completed: data?.completed || false
    })
    ...
    const handleSave = () => {
        let payload = {
            ...
            entity_id: formData?.related_id,
            entity_type: formData?.related_to,
            notes: formData.notes,
            relatedToId: formData?.related_id,
            relatedToType: formData?.related_to,
            related_to_id: formData?.related_id,
            related_to_type: formData?.related_to,
    ...
    return (<>
        <Header back={true} title={edit ? t("editActivity") : t("addActivity")} />
        <ScreenWrapper>
            <ScrollView>
                <View style={globalStyle.screen}>
                    <View style={[globalStyle.picker]}>
                        <Picker
                            ...
                            placeholder=''
                        >
                            {['Call', 'Email', 'Meeting', 'Note', 'Task', 'Reminder', 'WhatsApp'].map(type => (
